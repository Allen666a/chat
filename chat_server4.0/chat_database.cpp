...
void ChatDataBase::my_database_add_new_group(string user_id,string group_name){
	...
	str="insrt into group_user values('"+user_id+"','"+group_name+"','"+user_id+"');";
	sql=str.c_str();
...
void ChatDataBase::my_database_get_apply_list(Apply_list *applist,string user_id){
...
vector<string> ChatDataBase::my_database_get_group_number(string group_name){
	string str="select group_member from group_info where group_name='"+group_name+"';";
