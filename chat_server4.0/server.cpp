...
void Server::read_cb(struct bufferevent *bev, void *ctx){
	...
	}else if(cmd=="add_apply_info"){//要添加的好友在线  有好友申请信息，查询数据库，并返回
		server_get_apply_list(bev,val);
	}else if(cmd=="add_apply_deal"){
	//本用户处理申请信息，同意或者拒绝  同意在apply_list中设置1，拒绝设置0 然后回复对方消息同意还是拒绝，只有对方在线的时候才发送info
		server_deal_friend_apply(bev,val);
	}else if(cmd=="create_group"){
		server_create_group(bev,val);
	}else if(cmd=="add_group"){
		server_add_group(bev,val);
	}else if(cmd=="apply_add_group"){//群主在线，发给他一个消息让他查询看一下apply_group_list;
		//先不写
	...
	}else if(cmd=="get_friend_apply_info"){
		server_get_fiend_apply_info(bev,val);
	} 
...
//		}
	...
	chatdb->my_database_get_apply_group_list(apply_group_info,val["user_id"].asString());
...
void Server::server_deal_friend_apply(struct bufferevent* bev,Json::Value val){
	...
	if(chatlist->online_user.count(val["send_user_id"].asString())){
		...
		string  str=Json::FastWriter().write(vl);
		if(bufferevent_write(bv,str.c_str(),strlen(str.c_str()))<0){//回写客户端
	        	cout<<"server.cpp on line"<<__LINE__<<endl;
       		}
	}
	//没处理 2022、8、11 都在线，双方客户端都有好友刷新界面，返回什么呢？？待写	
}
void Server::server_add_group(struct bufferevent* bev,Json::Value val){
	...
	if(chatlist->online_user.count(tmp)){
		struct bufferevent*bv=chatlist->online_user[val["tmp"].asString()];
		Json::Value v;
...
void Server::server_group_appply_deal(struct bufferevent* bev,Json::Value val){
	...
	//如果申请者在线，就给他发送一个消息，让他刷新一下apply_group_list
}
void Server::server_private_chat(struct bufferevent* bev,Json::Value val){//私聊就是服务器转发消息的一个过程
	if(chatlist->online_user.count(val["user_to"].asString())){//对方在线马上发消息
		struct bufferevent* bv=chatlist->online_user[val["user_to"].asString()];
		
		string s=Json::FastWriter().write(val);
	
		if(bufferevent_write(bv,s.c_str(),strlen(s.c_str()))<0){//回写客户端
	        	cout<<"server.cpp on line"<<__LINE__<<endl;
       		}
...
void Server::server_group_chat(struct bufferevent* bev,Json::Value val){
	...
	for(int i=0;i<number.size();i++){
		if(chatlist->online_user.count(val[number[i]].asString())){
			//在线给其发消息
	
			struct bufferevent* bv=chatlist->online_user[val[number[i]].asString()];
			string s=Json::FastWriter().write(val);
			if(bufferevent_write(bv,s.c_str(),strlen(s.c_str()))<0){//回写客户端
	        		cout<<"server.cpp on line"<<__LINE__<<endl;
       			}
		}else{
			//不在线，可以存储起来，等用户上限的时候，加载一下
		}
	}
		//回复客户端发送成功
		Json::Value vl;
		vl["cmd"]="group_chat_reply";
		vl["result"]="sucess";
		string  str=Json::FastWriter().write(vl);
		if(bufferevent_write(bev,str.c_str(),strlen(str.c_str()))<0){//回写客户端
	        	cout<<"server.cpp on line"<<__LINE__<<endl;
...
void Server::server_send_file(struct bufferevent* bev,Json::Value val){
	...
	if(chatlist->online_user.count(val["to_user_id"].asString())){
		...
		//启动新线程，创建文件服务器
		int port=9090;//应该随机生成应该端口，并判断端口号在系统中是否被占用，没有被占用才能用 未修改，2022、8、16
...
void Server::server_get_fiend_apply_info(struct bufferevent* bev,Json::Value val){
